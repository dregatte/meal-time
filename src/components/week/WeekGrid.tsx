...
type SlotMap = Record<string, WeekSlotRow | undefined>;
...
export default function WeekGrid() {
  ...
  async function getSuggestions() {
    ...
    const emptySlots = DAYS.flatMap((d) =>
      MEALS.filter((m) => !slots[`${d.key}-${m}`]?.recipe_id).map((m) => ({ day: d.key, meal: m }))
    );
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => setWeekOffset((o) => o - 1)} className="text-sm text-gray-500 hover:text-gray-700 px-2 py-1">← Prev</button>
        <span className="text-sm font-medium text-gray-700">Week of {formatMonday(weekStart)}</span>
        <button onClick={() => setWeekOffset((o) => o + 1)} className="text-sm text-gray-500 hover:text-gray-700 px-2 py-1">Next →</button>
      </div>
          ...
          <div key={day} className="bg-white rounded-2xl p-3 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              {MEALS.map((meal) => {
                ...
                return (
                  <div key={meal} className="rounded-xl bg-gray-50 p-2 min-h-[52px]">
                    ...
                    {recipe ? (
                      <div className="flex items-start justify-between gap-1">
                        ...
                        <button onClick={() => assignSlot(day, meal, null)} className="text-gray-300 hover:text-red-400 shrink-0">
                          <X size={13} />
                        </button>
                      </div>
                    ) : (
      ...
      {picking && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end" onClick={() => setPicking(null)}>
          <div className="bg-white w-full max-h-[70vh] rounded-t-3xl overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-semibold">Pick a recipe</h2>
              <button onClick={() => setPicking(null)}><X size={20} /></button>
            </div>
            {recipes.length === 0 ? (
              <p className="p-6 text-sm text-gray-400 text-center">No recipes yet. Add some in the Recipes tab.</p>
            ) : (
              <ul className="divide-y divide-gray-50">
                {recipes.map((r) => (
                  <li key={r.id}>
                    <button className="w-full text-left px-4 py-3 text-sm hover:bg-orange-50 transition-colors"
